...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage4 = App\Models\Stage::where('stage_num','4')->get())
                @if($stage4->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stagefourtitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage4 as $st4)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st4->qty }}</td>
                            <td>{{ $st4->unit }}</td>
                            <td>{{ $st4->descriptions }}</td>
                            <td>{{ $st4->rate }}</td>
                            <td>{{ $st4->per }}</td>
                            <td>{{ number_format($st4->amt,2) }}</td>
                            <td>
                                @if($st4->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st4->id }} data-amt={{ $st4->amt }} data-paymenthod="{{ $st4->payment_method }}" data-estid="{{ $st4->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
