...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage2 = App\Models\Stage::where('stage_num','2')->get())
                @if($stage2->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stagetwotitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage2 as $st2)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st2->qty }}</td>
                            <td>{{ $st2->unit }}</td>
                            <td>{{ $st2->descriptions }}</td>
                            <td>{{ $st2->rate }}</td>
                            <td>{{ $st2->per }}</td>
                            <td>{{ number_format($st2->amt,2) }}</td>
                            <td>
                                @if($st2->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st2->id }} data-amt={{ $st2->amt }} data-paymenthod="{{ $st2->payment_method }}" data-estid="{{ $st2->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" data-id="{{ $st2->id }}">Pay</button>
                                    @else
