...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage8 = App\Models\Stage::where('stage_num','8')->get())
                @if($stage8->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stageseventitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage8 as $st8)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st8->qty }}</td>
                            <td>{{ $st8->unit }}</td>
                            <td>{{ $st8->descriptions }}</td>
                            <td>{{ $st8->rate }}</td>
                            <td>{{ $st8->per }}</td>
                            <td>{{ number_format($st8->amt,2) }}</td>
                            <td>
                                @if($st8->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st8->id }} data-amt={{ $st8->amt }} data-paymenthod="{{ $st8->payment_method }}" data-estid="{{ $st8->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal">Pay</button>
                                    @else
