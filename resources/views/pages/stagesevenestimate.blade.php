...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage7 = App\Models\Stage::where('stage_num','7')->get())
                @if($stage7->count() != 0)
                ...
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage7 as $st7)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st7->qty }}</td>
                            <td>{{ $st7->unit }}</td>
                            <td>{{ $st7->descriptions }}</td>
                            <td>{{ $st7->rate }}</td>
                            <td>{{ $st7->per }}</td>
                            <td>{{ number_format($st7->amt,2) }}</td>
                            <td>
                                @if($st7->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st7->id }} data-amt={{ $st7->amt }} data-paymenthod="{{ $st7->payment_method }}" data-estid="{{ $st7->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
