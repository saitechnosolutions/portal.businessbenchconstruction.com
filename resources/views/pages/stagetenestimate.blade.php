...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage10 = App\Models\Stage::where('stage_num','10')->get())
                @if($stage10->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stageeentitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage10 as $st10)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st10->qty }}</td>
                            <td>{{ $st10->unit }}</td>
                            <td>{{ $st10->descriptions }}</td>
                            <td>{{ $st10->rate }}</td>
                            <td>{{ $st10->per }}</td>
                            <td>{{ number_format($st10->amt,2) }}</td>
                            <td>
                                @if($st10->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st10->id }} data-amt={{ $st10->amt }} data-paymenthod="{{ $st10->payment_method }}" data-estid="{{ $st10->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
