...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage5 = App\Models\Stage::where('stage_num','5')->get())
                @if($stage5->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stagefivetitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage5 as $st5)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st5->qty }}</td>
                            <td>{{ $st5->unit }}</td>
                            <td>{{ $st5->descriptions }}</td>
                            <td>{{ $st5->rate }}</td>
                            <td>{{ $st5->per }}</td>
                            <td>{{ number_format($st5->amt,2) }}</td>
                            <td>
                                @if($st5->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st5->id }} data-amt={{ $st5->amt }} data-paymenthod="{{ $st5->payment_method }}" data-estid="{{ $st5->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
