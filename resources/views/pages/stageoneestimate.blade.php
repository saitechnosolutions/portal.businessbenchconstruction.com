...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage1 = App\Models\Stage::where('stage_num','1')->get())
                @if($stage1->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stageonetitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage1 as $st1)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st1->qty }}</td>
                            <td>{{ $st1->unit }}</td>
                            <td>{{ $st1->descriptions }}</td>
                            <td>{{ $st1->rate }}</td>
                            <td>{{ $st1->per }}</td>
                            <td>{{ number_format($st1->amt,2) }}</td>
                            <td>
                                @if($st1->payment_status == 0)
                                    <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st1->id }} data-amt={{ $st1->amt }} data-paymenthod="{{ $st1->payment_method }}" data-estid="{{ $st1->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
