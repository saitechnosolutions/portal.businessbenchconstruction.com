...
<section>
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                @if($stage6 = App\Models\Stage::where('stage_num','6')->get())
                @if($stage6->count() != 0)
                <h3 class="text-center" style="padding:25px 0px;">{{ $getestimate->stagesixtitle }}</h3>
                <table class="table table-bordered" >
                    ...
                    <tbody>
                        @php $i=1 @endphp
                    @foreach ($stage6 as $st6)
                        <tr>
                            <td>@php echo $i++ @endphp</td>
                            <td>{{ $st6->qty }}</td>
                            <td>{{ $st6->unit }}</td>
                            <td>{{ $st6->descriptions }}</td>
                            <td>{{ $st6->rate }}</td>
                            <td>{{ $st6->per }}</td>
                            <td>{{ number_format($st6->amt,2) }}</td>
                            <td>
                                @if($st6->payment_status == 0)
                                <button class="btn btn-danger btn-sm m-auto d-block getestimatepayment" data-id={{ $st6->id }} data-amt={{ $st6->amt }} data-paymenthod="{{ $st6->payment_method }}" data-estid="{{ $st6->estid }}" data-bs-toggle="modal" data-bs-target="#paymentmodal" >Pay</button>
                                    @else
